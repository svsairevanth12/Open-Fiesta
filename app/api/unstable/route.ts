  ...
  try {
    ...
    const sanitize = (msgs: unknown[]): OutMsg[] => {
      return msgs
        ...
        .map((m): OutMsg | null => {
          const role = typeof m.role === 'string' && ['user', 'assistant', 'system'].includes(m.role) 
    ...
    const trimmedMessages = sanitizedMessages.length > 10 ? sanitizedMessages.slice(-10) : sanitizedMessages;
    ...
    const totalTokensEstimate = processedMessages.reduce((sum, msg) => sum + estimateTokens(msg.content), 0);
    ...
    const response = await fetch(process.env.INFERENCE_API_ENDPOINT || 'https://inference.quran.lat/v1/chat/completions', {
      method: 'POST',
      headers: {
