    ...
    <div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-xl md:max-w-2xl lg:max-w-3xl xl:max-w-4xl 2xl:max-w-5xl mx-auto rounded-2xl border border-white/10 bg-zinc-900/90 p-6 md:p-7 lg:p-8 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-base md:text-lg lg:text-xl font-semibold tracking-wide">Add custom OpenRouter models</h3>
                <p className="text-xs md:text-sm text-zinc-400 mt-1">Add any model slug from OpenRouter. Selection is still capped at 5 in the picker.</p>
              </div>
              <button aria-label="Close" onClick={() => setOpen(false)} className="h-9 w-9 md:h-10 md:w-10 inline-flex items-center justify-center rounded-md bg-white/10 hover:bg-white/20">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-5 mb-3">
              <div className="space-y-1 sm:col-span-1">
                ...
                <input value={label} onChange={(e) => setLabel(e.target.value)} placeholder="DeepSeek: R1 Distill Qwen 14B (free)" className="w-full bg-black/40 border border-white/10 rounded-md px-3.5 py-2.5 text-sm md:text-base focus:outline-none focus:ring-1 focus:ring-white/30" />
              </div>
              <div className="space-y-1 sm:col-span-1">
                <label className="text-[11px] md:text-xs text-zinc-400">Model ID (slug)</label>
                <input value={slug} onChange={(e) => { setSlug(e.target.value); setValidState(null); setValidMsg(null); }} placeholder="provider/model:variant (e.g., deepseek/deepseek-r1:free)" className="w-full bg-black/40 border border-white/10 rounded-md px-3.5 py-2.5 text-sm md:text-base focus:outline-none focus:ring-1 focus:ring-white/30" />
              </div>
            </div>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={addCustom} disabled={validState !== 'ok'} className="px-3.5 py-2 rounded-md bg-[#e42a42] hover:bg-[#cf243a] disabled:opacity-60 disabled:cursor-not-allowed text-white text-sm md:text-base">Add Model</button>
              </div>
            </div>
            {err && <div className="mb-2 text-xs text-rose-300 inline-flex items-center gap-2"><AlertCircle size={14} /> {err}</div>}
            {validMsg && (
              <div className={`mb-2 text-xs inline-flex items-center gap-2 px-2 py-1 rounded-md border ${
                validState === 'ok' ? 'text-emerald-300 border-emerald-300/30 bg-emerald-400/10'
                : validState === 'fail' ? 'text-rose-300 border-rose-300/30 bg-rose-400/10'
                : 'text-amber-300 border-amber-300/30 bg-amber-400/10'
              }`}>
                {validState === 'ok' ? <Check size={14} /> : validState === 'fail' ? <AlertCircle size={14} /> : <AlertCircle size={14} />}
                <span>{validMsg}</span>
              </div>
            )}
                    ...
                    {customModels.map((m) => (
                      <tr key={m.id} className="border-t border-white/10 hover:bg-white/5">
                        <td className="px-3 py-2 align-top">
                          <div className="flex items-center gap-2">
                            <span className="truncate max-w-[260px] lg:max-w-[360px]">{m.label}</span>
                            <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/10 border border-white/15">custom</span>
                          </div>
                        </td>
                        <td className="px-3 py-2 align-top">
                          <div className="flex items-center gap-2 text-xs text-white/85">
                            <span className="truncate max-w-[360px] lg:max-w-[520px]" title={m.model}>{m.model}</span>
                            <button
                        ...
                        <td className="px-3 py-2 text-right align-top">
                          <button onClick={() => removeCustom(m.id)} className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded-md bg-white/10 border border-white/10 hover:bg-white/20">
