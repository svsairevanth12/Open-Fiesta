  ...
  return (
    <div className={`relative ${className}`}>
      <button
        ...
        className="flex items-center gap-2 px-3 py-2 bg-white/10 hover:bg-white/20 rounded-lg border border-white/20 text-sm transition-colors"
      >
        <Volume2 size={16} />
      ...
      {isOpen && (
        <>
          ...
          {/* Dropdown */}
          <div className="absolute top-full left-0 mt-1 w-64 bg-zinc-900 border border-white/20 rounded-lg shadow-xl z-50 overflow-hidden">
            <div className="p-2 border-b border-white/10">
              <h3 className="text-sm font-medium text-white">Select Voice</h3>
            </div>
              ...
              {VOICE_OPTIONS.map((voice) => (
                <button
                  ...
                  className={`w-full text-left p-3 hover:bg-white/10 transition-colors ${
                    voice.id === selectedVoice ? 'bg-white/15' : ''
                  }`}
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-xs text-zinc-500">{voice.gender}</div>
                  </div>
                </button>
              ))}
            </div>
