        ...
        {sidebarOpen ? (
          <>
            ...
            <div className="flex-1 overflow-y-auto space-y-1 pr-1">
              ...
              {threads.map(t => (
                <button key={t.id} onClick={() => onSelectThread(t.id)} className={`w-full text-left px-2 py-2 rounded-md text-sm border ${t.id === activeId ? 'bg-white/15 border-white/20' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}>
                  {t.title || 'Untitled'}
                </button>
              ))}
            </div>
        ...
        <div className="lg:hidden fixed inset-0 z-40">
          ...
          <div className="absolute left-0 top-0 h-full w-72 bg-zinc-900/90 border-r border-white/10 p-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <button aria-label="Close" onClick={onCloseMobile} className="h-8 w-8 inline-flex items-center justify-center rounded-md bg-white/10 hover:bg-white/20">
                <X size={16} />
              </button>
            </div>
            <button
            ...
            <div className="h-[70vh] overflow-y-auto space-y-1 pr-1">
              {threads.length === 0 && <div className="text-xs opacity-60">No chats yet</div>}
              {threads.map(t => (
                <button key={t.id} onClick={() => { onSelectThread(t.id); onCloseMobile(); }} className={`w-full text-left px-2 py-2 rounded-md text-sm border ${t.id === activeId ? 'bg-white/15 border-white/20' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}>
                  {t.title || 'Untitled'}
                </button>
              ))}
            </div>
